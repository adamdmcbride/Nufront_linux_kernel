...
static const u16 rt3261_dsp_441[][2] = {
	...
	{0x22ff, 0x3c83}, {0x22fa, 0x2487}, {0x2301, 0x0001},
};

#define RT3261_DSP_441_NUM (sizeof(rt3261_dsp_441) / sizeof(rt3261_dsp_441[0]))

static const u16 rt3261_dsp_16[][2] = {
	{0x22c8, 0x0026}, {0x22fa, 0x2487}, {0x2301, 0x0002},
};

#define RT3261_DSP_16_NUM (sizeof(rt3261_dsp_16) / sizeof(rt3261_dsp_16[0]))
//bard 10-1 s
static const u16 rt3261_dsp_8[][2] = {
	{0x22c8, 0x0026}, {0x22fa, 0x2487}, {0x2301, 0x0000},
};

#define RT3261_DSP_8_NUM (sizeof(rt3261_dsp_8) / sizeof(rt3261_dsp_8[0]))
...
static const u16 rt3261_dsp_aec_ns_fens[][2] = {
	...
	{0x2309, 0x0400}, {0x230a, 0x1b00}, {0x230c, 0x1000}, {0x230d, 0x0300},
	{0x2310, 0x0824}, {0x2325, 0x5000}, {0x2326, 0x0060}, {0x232f, 0x002c},
	...
	{0x23ea, 0x7ff0}, {0x23ed, 0x0300}, {0x2302, 0x0101}, {0x22fb, 0x0000},
};

#define RT3261_DSP_AEC_NUM \
	(sizeof(rt3261_dsp_aec_ns_fens) / sizeof(rt3261_dsp_aec_ns_fens[0]))
static const u16 rt3261_dsp_ec_normal_mode[][2] = {
	{0x232f, 0x002c},
};

#define RT3261_DSP_EC_NORMAL_NUM \
	(sizeof(rt3261_dsp_ec_normal_mode) / sizeof(rt3261_dsp_ec_normal_mode[0]))

static const u16 rt3261_dsp_ec_speaker_mode[][2] = {
	{0x232f, 0x0080},
};

#define RT3261_DSP_EC_SPEAKER_NUM \
	(sizeof(rt3261_dsp_ec_speaker_mode) / sizeof(rt3261_dsp_ec_speaker_mode[0]))
...
static int rt3261_dsp_write_table(struct snd_soc_codec *codec,
				  const unsigned short table[][2], int tab_num)
{
	unsigned int dsp_val = snd_soc_read(codec, RT3261_DSP_CTRL3);
	int i, ret;

	ret = rt3261_dsp_done(codec);
	ret |= snd_soc_write(codec, RT3261_GEN_CTRL3, 0x00e0);
	if (ret < 0)
		return -EBUSY;

	dsp_val &= ~(RT3261_DSP_R_EN | RT3261_DSP_CMD_MASK);
	dsp_val |= RT3261_DSP_W_EN | RT3261_DSP_CMD_MW;

	for (i = 0; i < tab_num; ++i) {
		ret = snd_soc_write(codec, RT3261_DSP_CTRL1, table[i][0]);
		ret |= snd_soc_write(codec, RT3261_DSP_CTRL2, table[i][1]);
 ...
 * Returns 0 for success or negative error code.
 */
static int rt3261_dsp_write(struct snd_soc_codec *codec, struct rt3261_dsp_param *param)
{
	unsigned int dsp_val = snd_soc_read(codec, RT3261_DSP_CTRL3);
	int ret;
 ...
 */
static unsigned int rt3261_dsp_read(struct snd_soc_codec *codec, unsigned int reg)
{
...
static int rt3261_dsp_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int rt3261_dsp_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt3261_priv *rt3261 = snd_soc_codec_get_drvdata(codec);
...
static int rt3261_dsp_ec_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt3261_priv *rt3261 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = rt3261->dsp_ec_mode;

	return 0;
}

static int rt3261_dsp_ec_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt3261_priv *rt3261 = snd_soc_codec_get_drvdata(codec);
	struct rt3261_dsp_param param;

	if (rt3261->dsp_ec_mode != ucontrol->value.integer.value[0]) {
		rt3261->dsp_ec_mode = ucontrol->value.integer.value[0];
		switch (rt3261->dsp_ec_mode) {
		case 1:
			rt3261_dsp_write_table(codec,
					       rt3261_dsp_ec_speaker_mode,
					       RT3261_DSP_EC_SPEAKER_NUM);
			break;
		default:
			rt3261_dsp_write_table(codec,
					       rt3261_dsp_ec_normal_mode, RT3261_DSP_EC_NORMAL_NUM);
			break;
		}
	}
...
static const SOC_ENUM_SINGLE_DECL(rt3261_src_rxdp_enum, RT3261_DSP_PATH1,
...
static const SOC_ENUM_SINGLE_DECL(rt3261_src_txdp_enum, RT3261_DSP_PATH1,
...
static const char *rt3261_dsp_ec_mode[] = { "Normal", "Speaker" };

static const SOC_ENUM_SINGLE_DECL(rt3261_dsp_ec_enum, 0, 0, rt3261_dsp_ec_mode);

static const char *rt3261_rxdp2_src[] = { "IF2_DAC", "Stereo_ADC" };

static const SOC_ENUM_SINGLE_DECL(rt3261_rxdp2_enum, RT3261_GEN_CTRL2,
...
static const char *rt3261_rxdp_src[] = { "RxDP2", "RxDP1" };

static const SOC_ENUM_SINGLE_DECL(rt3261_rxdp_enum, RT3261_DUMMY_PR3F, 10, rt3261_rxdp_src);

static const struct snd_kcontrol_new rt3261_rxdp_mux = SOC_DAPM_ENUM("RxDP sel", rt3261_rxdp_enum);

static const char *rt3261_rxdc_src[] = { "Mono_ADC", "Stereo_ADC" };

static const SOC_ENUM_SINGLE_DECL(rt3261_rxdc_enum, RT3261_GEN_CTRL2,
				  RT3261_RXDC_SRC_SFT, rt3261_rxdc_src);

static const struct snd_kcontrol_new rt3261_rxdc_mux = SOC_DAPM_ENUM("RxDC sel", rt3261_rxdc_enum);

static const char *rt3261_rxdp1_src[] = { "DAC1", "IF1_DAC" };

static const SOC_ENUM_SINGLE_DECL(rt3261_rxdp1_enum, RT3261_DUMMY_PR3F, 9, rt3261_rxdp1_src);
...
static const struct snd_kcontrol_new rt3261_dsp_snd_controls[] = {
	...
	SOC_ENUM_EXT("Echo Cancellation Mode Switch", rt3261_dsp_ec_enum,
		     rt3261_dsp_ec_get, rt3261_dsp_ec_put),
	/*
...
static int rt3261_dsp_patch_3(struct snd_soc_codec *codec)
{
	...
	if (ret < 0) {
		dev_err(codec->dev, "Fail to set DSP 3 bytes patch entrance: %d\n", ret);
		goto patch_err;
	}
...
static int rt3261_dsp_patch_2(struct snd_soc_codec *codec)
{
	...
	ret = rt3261_dsp_write(codec, &param);
	if (ret < 0) {
		dev_err(codec->dev, "Fail to set DSP 2 bytes patch entrance: %d\n", ret);
		goto patch_err;
	}
...
static int rt3261_dsp_patch(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_update_bits(codec, RT3261_PWR_DIG2, RT3261_PWR_I2S_DSP, RT3261_PWR_I2S_DSP);
	if (ret < 0)
		goto patch_err;
...
static int rt3261_dsp_conf(struct snd_soc_codec *codec)
{
	...
	ret = rt3261_dsp_write_table(codec, rt3261_dsp_init, RT3261_DSP_INIT_NUM);
	if (ret < 0) {
...
{
	int ret, tab_num;
	const unsigned short (*rate_tab)[2];
	...
	case 16000:
		...
		break;
	case 8000:
		rate_tab = rt3261_dsp_8;
		tab_num = RT3261_DSP_8_NUM;
		break;
	default:
		return -EINVAL;
		break;
	...
	ret = rt3261_dsp_write_table(codec, rate_tab, tab_num);
	if (ret < 0) {
...
{
	int ret, tab_num;
	const unsigned short (*mode_tab)[2];
	...
	ret = rt3261_dsp_write_table(codec, mode_tab, tab_num);
	if (ret < 0) {
...
static int rt3261_dsp_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		power_on--;
		if (!power_on) {
			snd_soc_update_bits(codec, RT3261_PWR_DIG2, RT3261_PWR_I2S_DSP, 0);
 ...
 */
static ssize_t rt3261_dsp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt3261_priv *rt3261 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt3261->codec;
	const unsigned short (*rt3261_dsp_tab)[2];
	unsigned int val;
	int cnt = 0, i, tab_num;
	...
	//bard 10-1 s
	rt3261_dsp_tab = rt3261_dsp_init;
			     ...
			     struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_ERR "register \"%s\" count=%d\n", buf, count);
	...
	printk(KERN_ERR "addr=0x%x val=0x%x\n", addr, val);
...
static DEVICE_ATTR(dsp_reg, 0777, rt3261_dsp_show, dsp_reg_store);
...
int rt3261_dsp_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_add_controls(codec, rt3261_dsp_snd_controls, ARRAY_SIZE(rt3261_dsp_snd_controls));
	...
	if (ret != 0) {
		dev_err(codec->dev, "Failed to create index_reg sysfs files: %d\n", ret);
		return ret;
	}
...
/*
   ...
   */
#ifdef RTK_IOCTL
int rt_codec_dsp_ioctl_common(struct snd_hwdep *hw, struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	...
	ret = snd_soc_update_bits(codec, RT3261_PWR_DIG2, RT3261_PWR_I2S_DSP, RT3261_PWR_I2S_DSP);
	if (ret < 0) {
		dev_err(codec->dev, "Failed to power up DSP IIS interface: %d\n", ret);
		goto err;
	}
