...
int bluesleep_can_sleep(void)
{
	...
	//                    , bsi->uport);
	return test_bit(BT_ASLEEP, &flags) && (bsi->uport != NULL);
}
...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
		...
		BT_SLEEP_DBG("setting to  sleep, host_wake %d",
			     gpio_get_value(bsi->host_wake));
		mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL * HZ * 2));
...
static void bluesleep_outgoing_data(void)
{
	...
	if (test_bit(BT_ASLEEP, &flags)) {	//(gpio_get_value(bsi->ext_wake)) {
...
static int of_nw53_bt_setup(struct platform_device *pdev,
			    struct bluesleep_info *bsi)
{
	...
	BT_SLEEP_DBG("register rfkill device return %d\n", ret);
	return ret;
}
#endif
...
{
	int ret;
	struct nw53_bt_platform_data *pdata;
...
#ifdef CONFIG_USE_OF
	if (of_nw53_bt_setup(pdev, bsi))
#endif
	{
		if (!pdev->dev.platform_data)
			return -ENOMEM;
		pdata = (struct nw53_bt_platform_data *)pdev->dev.platform_data;
		bsi->host_wake = pdata->int_gpio;
		bsi->ext_wake = pdata->bt_wake;
	}
...
static SIMPLE_DEV_PM_OPS(bluesleep_pm_ops, bluesleep_suspend, bluesleep_resume);
...
static int __init bluesleep_init(void)
{
	...
	sleep_dir = proc_mkdir("bluetooth", NULL);
	if (sleep_dir == NULL) {
