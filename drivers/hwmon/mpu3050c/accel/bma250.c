...
static int bma250_set_odr(void *mlsl_handle,
			  struct ext_slave_platform_data *pdata,
			  struct bma250_config *config, int apply, long odr)
{
	int result = INV_SUCCESS;
...
static int bma250_set_fsr(void *mlsl_handle,
			  struct ext_slave_platform_data *pdata,
			  struct bma250_config *config, int apply, long fsr)
{
	int result = INV_SUCCESS;
			    ...
			    const struct i2c_device_id *devid)
{
...
#ifdef CONFIG_OF
	struct device_node *of_mpu = NULL;
	const __be32 *node;
	u32 size;
	int i = 0;
	pdata = kzalloc(sizeof(struct ext_slave_platform_data), GFP_KERNEL);
	if (!pdata) {
		return -ENOMEM;
	}

	of_mpu = client->dev.of_node;
	node = of_get_property(of_mpu, "bus", NULL);
	if (node)
		pdata->bus = be32_to_cpup(node);
	printk(KERN_DEBUG "pdata->bus  = %d client->irq = %d\n", pdata->bus,
	       client->irq);

	node = of_get_property(of_mpu, "orientation", &size);
	...
	if (node) {
		...
		while (i < size) {
			pdata->orientation[i] = be32_to_cpup(node++);
			if (pdata->orientation[i] == 0xff)
				pdata->orientation[i] = -1;

			printk(KERN_DEBUG "pdata->orientation[%d]  = %d\n", i,
			       pdata->orientation[i]);
			i++;
		}
