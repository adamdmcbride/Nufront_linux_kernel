...
#else /* to let polldmp and hal work together */
	if (result) {
		mutex_unlock(&mpu->mutex);
		return -EBUSY;
	}
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	node = of_get_property(of_mpu, "int_config", NULL);
	if (node) {
		pdata->int_config = be32_to_cpup(node);
		printk(KERN_DEBUG "pdata->int_config = 0x%x client->irq = %d\n",
		       pdata->int_config, client->irq);
	}
	...
	if (client->irq) {
		...
		dev_WARN(&client->adapter->dev, "Missing %s IRQ\n", MPU_NAME);
	}
	return res;
