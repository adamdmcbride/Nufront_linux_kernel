...
if ((!op_mode && strstr(fw_path, "_p2p") != NULL) || (op_mode == 0x04) ||
	...
	glom = 15;
	bcm_mkiovar("bus:maxtxpktglom", (char *)&glom, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

	glom = 0;
	bcm_mkiovar("bus:txlazydelay", (char *)&glom, 4, iovbuf, sizeof(iovbuf));
	dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

	glom = 1;
	dhd_wl_ioctl_cmd(dhd, WLC_SET_FAKEFRAG, (char *)&glom, sizeof(glom), TRUE, 0);
