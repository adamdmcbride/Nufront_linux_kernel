...
//		firmware_path[0] = '\0';
		...
		/* added by Li Le for wifi direct workaround */
		DHD_ERROR(("%s: Damn! firmware_path is NULL", __FUNCTION__));
		strcpy(firmware_path, "/system/etc/firmware/fw_bcmdhd_p2p.bin\n");
		firmware_path[strlen(firmware_path)-1] = '\0';
		COPY_FW_PATH_BY_CHIP( dhd->pub.bus, fw_path, firmware_path);
		/* Direct return will cause FS processes exit */
		//return -EINVAL;
	}
...
#ifdef EXPORT_WIFI_MAC
static char mac_str[18];

int create_wifi_mac = 1;

static int mac_show(struct seq_file *seq, void *v)
{
	seq_printf(seq, "%s", mac_str);
	return 0;
}

static int mac_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mac_show, NULL);
}

static const struct file_operations mac_proc_ops = {
	.owner          = THIS_MODULE,
	.open           = mac_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
#define	WIFI_MAC_FILE	"wifi_mac"
	...
	filep = filp_open(WIFI_MAC_FILE, O_CREAT|O_WRONLY|O_TRUNC, 0666);
	if(IS_ERR(filep))
		DHD_ERROR(("%s: create wifi_mac file failed\n", __FUNCTION__));
	...
	fd = sys_open(WIFI_MAC_FILE, O_WRONLY, 0);
	if(fd >= 0) {
		memset(mac_str, 0, sizeof(mac_str));
		sprintf(mac_str, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n", 
				dhd->mac.octet[0], dhd->mac.octet[1], dhd->mac.octet[2],
		...
		DHD_INFO(("%s: WiFi mac_string %s", __FUNCTION__, mac_str));
		sys_write(fd, mac_str, sizeof(mac_str));
		DHD_INFO(("%s: WiFi MAC addr wrote\n", __FUNCTION__));
		sys_close(fd);
	}

	/* dude, cut that shit off */
	set_fs(old_fs);
#endif
	if(create_wifi_mac) {
		struct proc_dir_entry *ent;
		ent = proc_create(WIFI_MAC_FILE, 0, NULL, &mac_proc_ops);	
		if(!ent)
			DHD_ERROR(("%s: Failed to register with procfs.\n", __FUNCTION__));
		create_wifi_mac = 0;
	}
#endif
...
dhd_os_open_image(char *filename)
{
	 ...
	 */
	 if (IS_ERR(fp)) {
		 DHD_ERROR(("%s: check firmware & nvram first", __FUNCTION__));
		 fp = NULL;
	 }
