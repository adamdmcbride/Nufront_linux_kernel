...
#if DEBUG
#define TR1(fmt, args...) printk(KERN_DEBUG "i2c_designware: " fmt, ##args)
#define TR2(fmt, args...) printk(KERN_CRIT "i2c_designware: " fmt, ##args)
#else
#define TR1(fmt, args...) 
#define TR2(fmt, args...) 
#endif
...
static void i2c_dw_init(struct dw_i2c_dev *dev)
{
	...
	printk("i2c bus clock = %dKHZ\n", input_clock_khz);
//	u32 input_clock_khz = 66666;
	u32 ic_con, hcnt, lcnt, hold, cycle;
	...
	/* Caiculate cycle time, the unit is ns */
	cycle = 1000000 / input_clock_khz;  
	
	/* Set sda hold time as 200 ns */
	hold = 200 / cycle;
	writel(hold, dev->base + DW_IC_SDA_HOLD);
	...
	switch(i2c_bus_data->speed){
		case I2C_SPEED_FAST:
			ic_con = DW_IC_CON_MASTER | DW_IC_CON_SLAVE_DISABLE | DW_IC_CON_RESTART_EN | DW_IC_CON_SPEED_FAST;
			break;
		case I2C_SPEED_STD:
			ic_con = DW_IC_CON_MASTER | DW_IC_CON_SLAVE_DISABLE | DW_IC_CON_RESTART_EN | DW_IC_CON_SPEED_STD;
			break;
	}
...
static u32 get_max_divisor(u32 dividend, u32 base)
{
	u32 i;
...
static int __devinit dw_i2c_probe(struct platform_device *pdev)
{
	...
	}
	{
		...
		printk("i2c tx_fifo_depth = %d, rx_fifo_depth = %d", dev->tx_fifo_depth, dev->rx_fifo_depth);
...
static int dw_i2c_suspend(struct platform_device *pdev, pm_message_t mesg) //wangweilin_add 2011.5.26
{
        struct dw_i2c_dev *dev = platform_get_drvdata(pdev);

        TR0("dw_i2c_suspend\n");

 //       i2c_dw_read_save_reg_state(pdev);    //wangweilin_add 2011.5.27, read all registers value save to drvdata

        TR2("\n\n\n------------------------------------suspend-------------------------------------------------\n\n\n");
  //      i2c_dw_read_all_reg(pdev);    //wangweilin_add 2011.5.26, read all registers value
        TR2("\n\n\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n\n\n");
...
static int dw_i2c_resume(struct platform_device *pdev) //wangweilin_add 2011.5.26
{
        struct dw_i2c_dev *dev = platform_get_drvdata(pdev);
	...
	//writel(0, dev->base + DW_IC_INTR_MASK); /* disable IRQ */
        i2c_dw_init(dev);   //////////////////////////////////////////////////////////////////////////////////////
	writel(0, dev->base + DW_IC_INTR_MASK); /* disable IRQ */
//        i2c_dw_read_recover_reg_state(pdev);    //wangweilin_add 2011.5.27, recover dridata to all registers state
