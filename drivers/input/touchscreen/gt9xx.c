...
#endif
unsigned int GTP_RST_PORT;
unsigned int GTP_INT_PORT;
unsigned int GTP_MAX_HEIGHT = 1280;
unsigned int GTP_MAX_WIDTH = 720;
unsigned int GTP_INT_TRIGGER = 0;
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	...
	GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);   //add by zhanghui
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ip = of_get_property(np, "GTP_MAX_HEIGHT", NULL);
	if(ip)
		GTP_MAX_HEIGHT = be32_to_cpup(ip);
	ip = of_get_property(np, "GTP_MAX_WIDTH", NULL);
	if(ip)
		GTP_MAX_WIDTH = be32_to_cpup(ip);
	ip = of_get_property(np, "GTP_INT_TRIGGER", NULL);
	if(ip)
		GTP_MAX_WIDTH = be32_to_cpup(ip);
#else
...
#endif
	ts->irq_gpio = irq_to_gpio(client->irq);

	GTP_RST_PORT = ts->reset_gpio;
	GTP_INT_PORT = ts->irq_gpio;
	printk(KERN_ERR"GTP_RST_PORT = %d\n GTP_INT_PORT = %d\n",GTP_RST_PORT,GTP_INT_PORT);
...
static void goodix_ts_early_suspend(struct early_suspend *h)
{
	struct goodix_ts_data *ts;
	s8 ret = -1;
	GTP_DEBUG("#########Entern the goodix_ts_early_suspend ################\n");
...
static void goodix_ts_late_resume(struct early_suspend *h)
{
	struct goodix_ts_data *ts;
	s8 ret = -1;
	GTP_DEBUG("############Entern the goodix_ts_late_resume############\n");
	ts = container_of(h, struct goodix_ts_data, early_suspend);
