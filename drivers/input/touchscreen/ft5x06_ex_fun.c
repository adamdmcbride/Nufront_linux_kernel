...
#define DBUG 1
#if  DBUG
#define DbgPrintk(fmt, args...) printk(KERN_DEBUG  fmt, ##args)
#else
#define DbgPrintk
#endif

static int ft5x0x_i2c_Read(char * writebuf, int writelen, char *readbuf, int readlen);
...
int fts_ctpm_auto_clb(void)
{
	...
	for(i=0;i<100;i++)
	{
		...
		delay_qt_ms(200);
		DbgPrintk("[FTS] waiting calibration %d\n",i);

	}
	DbgPrintk("[FTS] calibration OK.\n");
	...
	msleep(300);
	DbgPrintk("[FTS] store CLB result OK.\n");
	return 0;
}
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if (i_ret != 0)
	{
		DbgPrintk("[FTS] upgrade failed i_ret = %d.\n", i_ret);
		//error handling ...
		//TBD
	}
	else
	{
		DbgPrintk("[FTS] upgrade successfully.\n");
...
int fts_ctpm_update_project_setting(void)
{
	...
	DbgPrintk("[FTS] Step 1: Reset CTPM test\n");
	...
	{
		DbgPrintk("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
	}
	else
	{
		return ERR_READID;
	...
	DbgPrintk("bootloader version = 0x%x\n", reg_val[0]);
	...
	DbgPrintk("[FTS] old setting: uc_i2c_addr = 0x%x, uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
			buf[0],  buf[2], buf[4]);
	for (i = 0; i < FTS_SETTING_BUF_LEN; i++)
	{
		if (i % 16 == 0)     DbgPrintk("\n");
		DbgPrintk("0x%x, ", buf[i]);
	}
	DbgPrintk("\n");
...
int fts_ctpm_auto_upg(void)
{
	...
	{
		msleep(100);
		DbgPrintk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
		...
		if (i_ret == 0)
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			DbgPrintk("[FTS] upgrade to new version 0x%x\n", uc_host_fm_ver);
		}
		else
		{
			DbgPrintk("[FTS] upgrade failed ret=%d.\n", i_ret);
		}
	}
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	auc_i2c_write_buf[0] = 0xcd;
	ft5x0x_i2c_Read(auc_i2c_write_buf, 1, reg_val, 1);
	DbgPrintk("[FTS] bootloader version = 0x%x\n", reg_val[0]);
	...
	delay_qt_ms(100);
	DbgPrintk("[FTS] Step 4: erase. \n");

	/*********Step 5:write firmware(FW) to ctpm flash*********/
	bt_ecc = 0;
	DbgPrintk("[FTS] Step 5: start upgrade. \n");
	dw_lenth = dw_lenth - 8;
	...
	for (j=0;j<packet_number;j++)
	{
		...
		{
			DbgPrintk("[FTS] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
		}
	}
	...
	ft5x0x_i2c_Read(auc_i2c_write_buf, 1, reg_val, 1);
	DbgPrintk("[FTS] Step 6:  ecc read 0x%x, new firmware 0x%x. \n", reg_val[0], bt_ecc);
	if(reg_val[0] != bt_ecc)
	{
		return ERR_ECC;
	}
...
static int __init ft5x0x_ex_init(void)
{
	DbgPrintk("entern ft5x06_ex_fun modle ");
	return 0;
}
 ...
 ***********************************************************************************************/
static void __exit ft5x0x_ex_exit(void)
{

	DbgPrintk("exit ft5x06_ex_fun modle ");
}

module_init(ft5x0x_ex_init);
module_exit(ft5x0x_ex_exit);
