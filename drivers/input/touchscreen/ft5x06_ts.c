...
static void ft5x0x_ts_pen_irq_work(struct work_struct *work)
{
	...
	//printk("[leo]---ft5x0x_ts_pen_irq_work---enter\n");
...
//	printk("[leo]---ft5x0x_ts_interrupt---enter\n");
...
for(uc_reg_value = 0; uc_reg_value <5; ++uc_reg_value){
	...
	if(err < 0){
		printk("[leo]---tp test err---");
	}else{
			printk("[leo]---tp test id=0x%x--\n",temp);
	}
}
